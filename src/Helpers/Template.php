...
class Template {
     ...
     * final ser? perfeitamente fiel ao arquivo original, isto ?, todas as tabulAÇÕES
     ...
     * @param     booelan $accurate		true para fazer substitui??o fiel das tabulAÇÕES
     */
    ...
    public function addFile($varname, $filename) {
        if (!in_array($varname, $this->vars))
            throw new InvalidArgumentException("addFile: var $varname n?o existe");
    ...
    private function loadfile($varname, $filename) {
        if (!file_exists($filename))
            throw new InvalidArgumentException("arquivo $filename n?o existe");
        // Reading file and hiding comments
        $str = preg_replace("/<!---.*?--->/smi", "", file_get_contents($filename));
        $blocks = $this->recognize($str, $varname);
        if (empty($str))
            throw new InvalidArgumentException("arquivo $filename est? vazio");
    ...
    private function setBlock($parent, $block) {
        ...
        if (1 !== preg_match($reg, $str, $m))
            throw new UnexpectedValueException("bloco $block est? mal formado");
        $this->setValue($name, '');
    ...
    public function block($block, $append = true) {
        if (!in_array($block, $this->blocks))
            throw new InvalidArgumentException("bloco $block n?o existe");
        if ($append)
